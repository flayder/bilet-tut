import { Component, Input, OnInit } from '@angular/core';
import { GetEventLocalStorageData } from 'src/app/exports/functions/GetEventLocalStorageData';
import { RandomStringFunction } from 'src/app/exports/functions/RandomStringFunction';
import { IEventAreaCategory } from 'src/app/exports/interfaces/IEventAreaCategory';
import { EventService } from 'src/app/services/event.service';
import { MessageService } from 'src/app/services/message.service';
import { ModalService } from 'src/app/services/modal.service';

@Component({
  selector: 'tr[data-app-area-category-table-item]',
  templateUrl: './area-category-table-item.component.html',
  styleUrls: ['./area-category-table-item.component.css']
})
export class AreaCategoryTableItemComponent implements OnInit {

  @Input() item: IEventAreaCategory

  constructor(
    private modal$: ModalService,
    private event$: EventService,
    private message$: MessageService
  ) { }

  ngOnInit(): void {

  }

  changeCheckbox(event: any) {
    this.event$.updateAreaCategory(this.item.id, {active: event.target.checked})
      .subscribe(response => {
        this.message$.handle('Категория цен успешно обновлена', 'success')
      })
  }

  edit(id: number, event: any) {
    event.preventDefault()
    this.modal$.open('account-category-tiny', {
      edit: true,
      id
    })
  }

  delete(id: number, event: any) {
    event.preventDefault()
    const id$: any = RandomStringFunction()
    this.modal$.buttonClick.subscribe((i: any) => {
      if(i == id$) {
        this.event$.deleteAreaCategory(id).subscribe(response => {
          setTimeout(() => {
            this.modal$.open('notification-tiny', {
              type: 'success',
              text: 'Категория цен успешно удалена',
              buttonText: false
            })
          }, 200)
          const params: any = {event__pk: false}
          const items: any = GetEventLocalStorageData()

          if(items.id > 0)
            params.event__pk = items.id

          this.event$.getAreaCategories(params)
        })
      }
    })
    this.modal$.open('notification-tiny', {
      type: 'delete',
      id: id$,
      text: 'Вы действительно хотите удалить категорию?',
      buttonText: 'Удалить'
    })
  }

}
